...
import tempfile
from prompt_repos import prompt_template_stuff as prompt_template
...
async def main():
    ...
    st.title("员工关怀All-in-one")
    st.write("""
             本问答系统已初步训练某大型制造企业内部员工手册，可支持您直接问答 \n \n
    ...
    # 默认选择“否”
    upload_option = st.selectbox("您想上传员工手册吗?", ["否", "是"], index=0)
    
    if upload_option == "是":
        uploaded_file = st.file_uploader("选择一个员工手册", type="md")
    ...
    if 'ready' in st.session_state and st.session_state['ready']:
        ...
        if 'generated' not in st.session_state:
            st.session_state['generated'] = ["准备就绪! 让AI来帮助您更好地使用员工手册"]
            st.session_state['image_ref'] = []
            ...
            if submit_button and user_input:
                ...
                if 'hybrid_retriever' in locals():
                    ...
                    if len(search_result) == 0:
                        ...
                        output = chain.run(input_documents=search_result[0:4], question=user_input) 
